...
JNIEXPORT void JNICALL Java_com_xue_song_ffmpeg_FPlayer_doOutput
  (JNIEnv * env, jobject obj, jstring jstr_input, jstring jstr_oupt){
    ...
    for(;i<avFormatContext->nb_streams;i++){
          if(avFormatContext->streams[i]->codec=AVMEDIA_TYPE_VIDEO){
              v_stream_idx=i;
              break;
          }
    }
   ...
   LOGI("文件格式%s",avFormatContext->iformat->name);
   LOGI("视频时长%ld",(avFormatContext->duration)/1000000);
   LOGI("解码宽%d、高%d",avCodecContext->width,avCodecContext->height);
  ...
  //用于存储 解码后的数据
   ...
   u_int8_t  * out_buffer=av_malloc(avpicture_get_size(AV_PIX_FMT_YUV420P,avCodecContext->width,avCodecContext->height));
   //初始化 缓冲区
   avpicture_fill((AVPicture *)avFrame,out_buffer,AV_PIX_FMT_YUV420P,avCodecContext->width,avCodecContext->height);
   //获取转码 上下文
